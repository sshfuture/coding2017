...
public class ArrayUtil {
	...
	private boolean checkPrimes(int paramI){
		for(int i=2;i<paramI;i++)
		{
			if(paramI%i == 0)
			{
				return false;
	...
	private boolean checkPerfectNumbers(int paramI){

		int [] array = new int [paramI];
		...
		for(int i=2;i<paramI;i++)
		{
			...
			int tmpI = paramI/i;
			if(arraySb.indexOf(","+i+",") == -1)
			{
				array[index++] = i;
				arraySb.append(","+i+",");
			}

			if(arraySb.indexOf(","+tmpI+",") == -1)
			{
				array[index++] = tmpI;
				arraySb.append(","+tmpI+",");
			}
